...
export function Canvas({ onOpenAgent }: CanvasProps) {
  ...
  const { canvas, selectedNodeId, selectedEdgeId, connectingFrom, setSelectedNode, setSelectedEdge, setConnectingFrom, addNode, addEdge, updateEdgeType, updateNodePos, updateNodeData, deleteNode, triggerGenerate, deleteEdge, loadModelPricing, loadEnabledModels, loadRuntimeModels, sendNodePositions } = useEditorStore();
  ...
  // 切换画布时清空远程协作状态
  useEffect(() => {
    useEditorStore.setState({ onlineUsers: [], remoteCursors: {} });
  }, [canvas?.id]);
...
loadRuntimeModels();
  ...
  const handleCanvasMouseMove = useCallback((e: MouseEvent) => {
    ...
    const s = scaleRef.current;
    const o = offsetRef.current;
    const canvasX = (e.clientX - rect.left - o.x) / s;
    const canvasY = (e.clientY - rect.top - o.y) / s;
    const send = useEditorStore.getState().collabSend;
    if (send) {
      send({
        type: 'cursor_move',
        canvas_id: cv.id,
    ...
    >
      ...
      >
        ...
        {Object.entries(remoteCursors).map(([userId, cursor]) => (
          ...
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{ marginLeft: -2, marginTop: -2 }}>
              <path
                d="M3 3l7.5 18.5L12.5 13.5L18.5 12.5L3 3z"
            ...
            {cursor.username && (
              <span
                className="ml-3 -mt-1 px-1.5 py-0.5 rounded text-[10px] font-semibold whitespace-nowrap shadow-sm"
                style={{ backgroundColor: cursor.color || '#ef4444', color: '#fff' }}
              >
                {cursor.username}
              </span>
            )}
      ...
      <div className="absolute bottom-6 left-6 flex items-center gap-2 glass rounded-2xl px-4 py-2 text-xs text-theme-sub shadow-soft">
        ...
        {onlineUsers.length > 0 && (
          <>
            <span className="mx-1 text-panel-border">|</span>
            <span className="flex items-center gap-1" title={onlineUsers.map((u) => u.username || u.user_id).join(', ')}>
