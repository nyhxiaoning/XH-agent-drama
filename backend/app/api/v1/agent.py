...
async def short_drama_generate_all(req: ShortDramaGenerateAllReq, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for n, task_type, cost in to_submit:
        ...
        if n.node_type.value == NodeType.VIDEO.value:
            ...
            params["durationSec"] = node_config.get("duration") or node_config.get("duration_seconds") or 5
            params["resolution"] = node_config.get("resolution")
            params["sound"] = node_config.get("sound")
            params["watermark"] = node_config.get("watermark")
            params["start_end_frame"] = node_config.get("start_end_frame")
